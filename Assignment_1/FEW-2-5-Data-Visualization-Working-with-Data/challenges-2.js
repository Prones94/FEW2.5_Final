...
function filterByProperty(data, property, value) {
	return data.filter(p => p.fields[value] !== undefined).filter(p => p.fields[property] === value)
}
...
function filterNullForProperty(data, property) {
	return data.filter(p => p.fields[property] !== undefined).map(p => p.fields[property])
}
